...
def _validate_entries_shape(entries: list) -> None:
    """
    校验单条分录的金额形态是否合法。
    ...
    for entry in entries:
        ...
        if debit < 0 or credit < 0:
            raise ValueError("分录金额不能为负数。")

        if (debit == 0 and credit == 0) or (debit > 0 and credit > 0):
            raise ValueError("每条分录必须且只能有借方或贷方一侧有金额。")


def _validate_accounts_exist_and_active(db: Session, entries: list) -> None:
    """
    校验分录引用的科目必须真实存在且处于启用状态。
    """
    valid_account_ids = {
        account_id
    ...
    if not valid_account_ids:
        raise ValueError("当前没有启用中的科目，无法创建凭证。")

    for entry in entries:
        if entry.account_id not in valid_account_ids:
            raise ValueError(f"凭证分录引用了不存在或未启用的科目 ID：{entry.account_id}")
...
def create_voucher(db: Session, data) -> Voucher:
    ...
    - 借贷必须严格平衡
    """
    if not str(data.memo).strip():
        raise ValueError("凭证摘要不能为空。")

    _validate_entries_shape(data.entries)
    _validate_entries_balance(data.entries)
    _validate_accounts_exist_and_active(db, data.entries)
